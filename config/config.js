 ...
 */
var getGlobbedPaths = function(globPatterns, excludes, ignores) {
    // URL paths regex
    ...
    if (_.isArray(globPatterns)) {
        globPatterns.forEach(function(globPattern) {
            output = _.union(output, getGlobbedPaths(globPattern, excludes, ignores));
        });
                ...
                _.remove(files, function(file) {
                    if (_.isArray(ignores)) {
                        for (var i in ignores) {
                            if (ignores.hasOwnProperty(i)) {
                                var r = minimatch(file, ignores[i]);
                                
                                if (r) {
                                    //console.log("minimatch " + file + " with " + ignores[i] + " : " + r);
                                    return true;
                                }
                            }
                        }
                    } else {
                        if (minimatch(file, ignores[i])) {
                            return true;
                        }
                    }
                    return false;
...
var initGlobalConfigFiles = function(config, assets) {
    ...
    config.files.client.js = getGlobbedPaths(assets.client.lib.js, 'public/', assets.client.lib.ignorejs).concat(getGlobbedPaths(assets.client.js, ['public/'], assets.client.ignorejs));

    // Setting Globbed css files
    config.files.client.css = getGlobbedPaths(assets.client.lib.css, 'public/', assets.client.lib.ignorecss).concat(getGlobbedPaths(assets.client.css, ['public/'], assets.client.ignorecss));
